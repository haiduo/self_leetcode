'''https://zhuanlan.zhihu.com/p/40976906'''
import numpy as np
import random
import cv2


def non_max_suppress(predicts_dict, conf_thres, num_thresh):
	for object_name, bbox in predicts_dict.items():  # 对每一个类别分别进行NMS；一次读取一对键值（即某个类别的所有框）
		bbox_array = np.array(bbox, dtype=float)
		bbox_array = bbox_array[bbox_array[:,4] >= conf_thres]
		#下面分别获取框的左上角坐标（x1，y1），右下角坐标（x2，y2）及此框的置信度；这里需要注意的是图像左上角可以看做
		# 坐标点（0,0），右下角可以看做坐标点（1,1），也就是说从左往右x值增大，从上往下y值增大
		x1 = bbox_array[:, 0]
		y1 = bbox_array[:, 1]
		x2 = bbox_array[:, 2]
		y2 = bbox_array[:, 3]
		...
		#argsort函数返回的是数组值从小到大的索引值,[::-1]表示取反。即这里返回的是数组值从大到小的索引值
		areas = (x2 - x1 + 1) * (y2 - y1 + 1)
		#当前类所有框的面积(python会自动使用广播机制，相当于MATLAB中的.*即两矩阵对应元素相乘)；x1=3,x2=5,
		# 习惯上计算x方向长度就是x=3、4、5这三个像素，即5-3+1=3，而不是5-3=2，所以需要加1
		keep = []
		...
		while order.size > 0:
			...
			#注意这里都是采用广播机制，同时计算了置信度最高的框与其余框的IoU
			inds = np.where(iou <= num_thresh)[0]
			#保留iou小于等于阙值的框的索引值
			order = order[inds + 1]
...
if __name__ == "__main__":
	...
	for object_name, bbox in predicts_dict.items():
		...
		# 在窗口中显示图像;注意这里的窗口名字如果不是刚刚创建的窗口的名字则会自动创建一个新的窗口并将图像显示在这个窗口
	...
	conf_thres = 0.1
	num_thresh = 0.35
	predicts_dict_nms = non_max_suppress(predicts_dict, conf_thres, num_thresh)
	for object_name, bbox in predicts_dict_nms.items():
		for box in bbox:
			x1, y1, x2, y2, score = int(box[0]), int(box[1]), int(box[2]), int(box[3]), box[-1]
			y_text = int(random.uniform(y1, y2))
			# uniform()是不能直接访问的，需要导入 random 模块，然后通过 random 静态对象调用该方法。uniform()
